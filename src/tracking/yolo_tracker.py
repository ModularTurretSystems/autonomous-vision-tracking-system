...
class YoloTargetDetector:
    ...
    def __init__(
            ...
            classes: Optional[List[float]] = None, #None = all classes, but [0] is only person
    ...
    def aim_from_bbox(self, det: BBoxDet) -> Aim:
        ...
        if self.aim_point == "head":
            return Aim(
                x = float((x2+x1) * 0.5),
                y = float((y2 - y1) * 0.18 + y1)
            )
        #torso
        return Aim(
            x = float((x1 + x2) * 0.5),
            y = float(y1 + 0.45 * (y2 - y1))
        )
    ...
    @staticmethod
    def deadzone_rect(
        frame_bgr: Tuple[int, int, int],
        deadzone_frac: Tuple[float, float] = (0.12, 0.12)
